...
- `orders_workspace` owns a compact syntax guide and non-stretching power cards with a shared-size rich canonical/editor surface, marks unparseable original lines in red, owns one inline text editor per configured power, warning expansion, optional finalisation controls and collection of pending text before an order-overlay preview; card styling is scoped to the outer frame so nested text and stacked widgets retain stable geometry.
- `new_game_workspace` edits game metadata, optional order finalisation and game-specific starting state inside the main-window stack.
- `map_manager_workspace` selects reusable maps and starts existing-map or imported-SVG configuration.
- `map_wizard` coordinates the independently accessible Definition, Powers & start and Placement pages, keeps their immutable draft in sync, displays the effective adjacency graph beside the complete searchable YAML source, edits territory names, combines independently selectable placement layers using the fixed unit symbols, previews and positions army/fleet hold underlines, moves and rotates named-coast labels, drags every anchor and validates or saves from any tab; its current-game mode shows the complete editor, warns before replacing the all-phases private map and offers reusable-map promotion actions.
