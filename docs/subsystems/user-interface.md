...
- `orders_workspace` owns compact non-stretching power cards with a shared-size canonical/editor surface, one inline text editor per configured power, warning expansion, final toggles, the unfinalised-power filter and collection of pending text before an order-overlay preview; card styling is scoped to the outer frame so nested text and stacked widgets retain stable geometry.
