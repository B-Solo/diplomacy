...
- `map_wizard` presents independently accessible configuration tabs, links classified rows with SVG hover highlights, displays a large readable army/fleet-anchor adjacency graph over a faded map, merges focused power and starting-setup YAML into the complete document, combines independently selectable placement layers, previews effective unit assets at constrained scale, drags every anchor and validates or saves their shared immutable draft from any tab.
