...
- `orders_workspace` owns compact non-stretching power cards with a shared-size rich canonical/editor surface, marks unparseable original lines in red, owns one inline text editor per configured power, warning expansion, optional finalisation controls and collection of pending text before an order-overlay preview; card styling is scoped to the outer frame so nested text and stacked widgets retain stable geometry.
