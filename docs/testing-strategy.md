...
Map-setup tests reopen an existing reusable map in the main-window stack, link classified rows and map highlights, verify land and sea topology nodes use their army and fleet anchors, combine placement layers without artificial centre markers, distinguish trackpad panning from mouse-wheel and pinch zooming, drag ordinary and split-coast anchors, preview effective unit assets, save the edited map and verify that an existing game's private map snapshot is unchanged.
