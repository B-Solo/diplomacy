...
- `DONT_SKIP_PHASES` produced every movement, retreat and adjustment phase even when no decisions were required.
...
Adjudication uses a separately reconstructed game with `NO_CHECK` and `DONT_SKIP_PHASES` enabled.
